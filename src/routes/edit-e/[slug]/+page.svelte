<script lang="ts">
    ...
    const { existingSlug, eventId } = data
...
<form method="POST" action="?/editEvent" class="flex flex-col w-64">
    ...
    {#if $errors.end_time}
        <p class="text-red-500">{$errors.end_time}</p>
    {/if}
    ...
    {:else if hasBeenSubmitted}
        <p class="text-yellow-600">Updating event...</p>
    {/if}
    {#if $errors._errors}
